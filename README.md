...
* **Sets**: Every time the player begins a new game, the code puts random room numbers into a Redis Set until there are 3 members there.  It uses the `SADD` and `SCARD` commands for this.  These are the rooms that will hold the keys the player needs to escape for this game.  We use the `SISMEMBER` commmand to check if a room number should have a key in it when sending the room map to the front end.
